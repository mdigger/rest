...
// Handler является любая функция, которая принимает Context. Если в результате
// возвращается ошибка и ответа от сервера еще не передавалось, то эта ошибка
// будет возвращена в качестве ошибки сервера.
//
// Функция автоматически отслеживает типы ошибок и для некоторых из них может
// изменять статус возврата ответа. В частности, если ошибка соответствует
// os.IsNotExist, то вернется 404 ошибка. А если соответствует os.IsPermission,
// то — 403.
//
// Если Handler используется совместно с ServeMux, то вы можете самостоятельно
// переопределить коды возврата для ошибок, задав функцию ServeMux.Errors.
...
func handler(handlr interface{}) Handler {
	...
	case http.HandlerFunc:
		// стандартную http-функцию оборачиваем в простой обработчик
		return func(c *Context) error {
			c.sended = true // мы не управляем отдачей содержимого ответа
			// т.к. стандартные обработчики не могут получить доступ к
			// именованным параметрам пути, то добавляем их как именованные
			// параметры в запрос
			if len(c.Params) > 0 {
	...
	case func() http.Handler:
		return handler(h())
	case http.Handler:
		// стандартный обработчик используем как простую http-функцию
