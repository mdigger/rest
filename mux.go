...
type ServeMux struct {
	...
	// Вы можете определить свою функцию, которая будет, в зависимости от ошибки,
	// возвращать разные коды завершения запроса и текст, который будет возвращаться.
	// Например, для всех ошибок mgo.ErrNotFound устанавливать код 404. Если же вы
	// хотите игнорировать данную ошибку, то проще всего в этом случае вернуть nil.
	Errors func(err error) *HTTPError
...
// Handles добавляет обработчики для указанных путей и методов. При указании путей можно
// использовать параметры, которые задаются символом ':' перед его названием. Так же можно
// использовать "завершающий" параметр, который "заберет"" в себя всю оставшуюся часть пути. Такой
// параметр задается символом '*' и должен обязательно идти последним в пути, в противном случае
// случится panic.
//
// В случае, если указанный обработчик не может быть приведен к типу поддерживаемых обработчиков,
// количество элементов пути больше 32767 или параметр со звездочкой используется не в конце пути,
// то случается panic.
func (m *ServeMux) Handles(paths Paths) {
	for path, methods := range paths { // перебираем все пути
		...
		mh, _ := m.router.lookup(path)  // получаем список уже заданных обработчиков
		...
		for method, h := range methods { // перебираем все методы
			...
			handlers[strings.ToUpper(method)] = handler(h) // приводим типы к обработчику
		}
		if err := m.router.add(path, handlers); err != nil { // добавляем обработчики
			panic(err)
		}
	}
}

// Handle добавляет новый обработчик для указанного метода и пути. Если обработчик не может быть
// приведен к поддерживаемому типу, то возникает panic.
//
// На сегодняшний момент поддерживаются следующие типы обработчиков: Handler, http.Handler и
// http.HandlerFunc.
func (m *ServeMux) Handle(method, path string, handler interface{}) {
	m.Handles(Paths{path: Methods{method: handler}}) // сводим задачу к первоначальной
}
