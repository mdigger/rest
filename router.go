...
// параметрами, а в младших — со статическими путями.
...
next:
	for _, record := range records { // перебираем все записи с обработчиками
		...
		for i, part := range record.parts { // перебираем все части пути, заданные в обработчике
			...
			if part != parts[i] { // сравниваем, что статическая часть пути совпадает с запрашиваемой
				continue next // не совпало — переходим к следующему обработчику
			}
		}
